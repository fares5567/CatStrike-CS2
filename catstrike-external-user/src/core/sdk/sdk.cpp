...
sdk_c::bounds_c::data_t sdk_c::bounds_c::get_data( const bones_c::data_t& bone_data ) const
{
	...
	const auto head_pos = bone_data.get_position( bone::head );
	const auto pelvis_pos = bone_data.get_position( bone::pelvis );
	
	
	const auto head_screen = parent->w2s.project( head_pos );
	const auto pelvis_screen = parent->w2s.project( pelvis_pos );
	
	
	const auto height = std::abs( pelvis_screen.y - head_screen.y );
	const auto width = height * 0.5f;
	
	
	const auto center_x = head_screen.x;
	const auto top_y = head_screen.y;
	const auto bottom_y = pelvis_screen.y;
	
	
	const auto padding_x = width * 0.1f;
	const auto padding_y = height * 0.05f;
	
	return {
		center_x - (width * 0.5f) - padding_x,  // min_x
		center_x + (width * 0.5f) + padding_x,  // max_x
		top_y - padding_y,                       // min_y  
		bottom_y + padding_y                     // max_y
	};
}

sdk_c::bounds_c::data_t sdk_c::bounds_c::get_data( std::uintptr_t m_p_game_scene_node ) const
{
	
	const auto player_pos = g->memory.read<math::vector3>( m_p_game_scene_node + offsets::m_vec_origin );
	
	
	const auto head_pos = math::vector3{ player_pos.x, player_pos.y, player_pos.z + 70.0f };
	
	
	const auto feet_screen = parent->w2s.project( player_pos );
	const auto head_screen = parent->w2s.project( head_pos );
	
	
	const auto height = std::abs( feet_screen.y - head_screen.y );
	const auto width = height * 0.5f;
	...
	return {
		center_x - (width * 0.5f),  // min_x
		center_x + (width * 0.5f),  // max_x
		head_screen.y,               // min_y (Kopf oben)
		feet_screen.y                // max_y (Füße unten)
	};
}
